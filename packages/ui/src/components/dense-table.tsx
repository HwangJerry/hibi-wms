...
export function DenseTable<T>({
  ...
  return (
    <div className={cx("border border-border rounded-md overflow-hidden", className)}>
      <div className={cx("flex items-center bg-surface-2/70 border-b border-border", "h-8 px-3 text-[11px] font-semibold uppercase tracking-wide text-text-secondary", headerClassName)}>
        {columns.map((column) => (
          <div
            key={column.id}
            className={cx(
