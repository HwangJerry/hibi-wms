...
- `pnpm dev` runs web + api + realtime against the Postgres instance reachable from
  this host. Seed with `pnpm db:seed` (two users + sample data).
- Postgres normally runs in the connected k3s cluster. Apply
  `infra/k8s/postgres.yaml` after creating the `hibi-postgres` Secret in the
  `hibi-portal` namespace. The manifest exposes Postgres through NodePort `30432`.
- From this host, use the server-main Tailscale address in `.env`:
  `postgresql://hibi:<password>@100.74.225.115:30432/hibi_portal?schema=public`.
  The same URL can be exported as `DATABASE_URL` before running Prisma commands.
